...
export const shouldPromptForTemplate = (templates: string[]): boolean => 
  templates.length > 1;
...
export const promptForTemplate = (config: MdgConfig) => 
...
export const processAnswerValue = (key: string, value: any): { key: string; processedValue: string } => {
...
export const collectAnswers = (prompts: Record<string, PromptConfig>) => 
...
export const promptForVariables = (template: TemplateConfig) => 
  async (): Promise<Result<GeneratorOptions, Error>> => {
...
export const getTemplateByName = (config: MdgConfig, templateName: string): Result<TemplateConfig, Error> => {
...
const runInteractiveModeFunc = (config: MdgConfig) => 
  async (): Promise<Result<GeneratorOptions, Error>> => {
